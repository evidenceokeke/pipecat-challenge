...
async def handle_user_idle(processor, retry_count):
    if retry_count == 1:
        logger.debug("Silence detected. Checking with participant...")
...
async def main(
    ...
    async def on_first_participant_joined(transport, participant):
        ...
        logger.add("call_summary.log",
                   format="{time:YYYY-MM-DD HH:mm:ss.SSS zz} | <level>{level: <8}</level> | <yellow>Line {line: >4} ({file}):</yellow> <b>{message}</b>")
